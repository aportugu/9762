...
fit1 <- cph(Surv(data1$Days.to.death.or.DLC, data1$death) ~ Tumor.burden, data=data1)
df1 <- data.frame(Predict(fit1, Tumor.burden, fun=function(x) exp(x) ) )


fit2 <- cph(Surv(data1$Days.to.death.or.relapse.or.DLC, data1$relapse.or.death) ~ Tumor.burden, data=data1)
df2 <- data.frame(Predict(fit2, Tumor.burden, fun=function(x) exp(x) ) )

ggplot() + 
  geom_line(data=df1, aes(x=Tumor.burden,y=yhat, color = "OS")) + geom_ribbon( data=df1, aes(x=Tumor.burden,ymin=lower, ymax=upper),alpha=0.10)+
  geom_line(data=df2, aes(x=Tumor.burden,y=yhat, color ="PFS" )) + geom_ribbon(data = df2, aes(x=Tumor.burden,ymin=lower, ymax=upper),alpha=0.10)+
